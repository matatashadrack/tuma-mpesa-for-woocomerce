...
require_once plugin_dir_path(__FILE__) . 'includes/class-tuma-pdf-receipt.php';
...
function init_tuma_payments_gateway() {
    ...
    class WC_Tuma_Payments_Gateway extends WC_Payment_Gateway {
        ...
        public $enable_website_product_sync;
        public $enable_sms;
        public $sms_token;
        ...
        public function __construct() {
            ...
            $this->enable_website_product_sync = 'yes' === $this->get_option('enable_website_product_sync', 'yes');
            ...
            add_action('woocommerce_api_tuma_pdf_receipt', array($this, 'download_pdf_receipt'));
            ...
            add_action('woocommerce_order_details_after_order_table', array($this, 'show_receipt_download'), 20);
        }
        ...
        public function init_form_fields() {
            $this->form_fields = array(
                ...
                'enable_product_sync' => array(
                    ...
                    'description' => 'When enabled, product details and stock are refreshed from the primary Tuma POS catalog on the configured schedule.',
                    'default'     => 'no',
                    'desc_tip'    => true,
                ),
                'product_sync_interval' => array(
                    'title'             => 'Stock Sync Frequency',
                    'type'              => 'number',
                    'description'       => 'How often POS product and stock data is refreshed in WooCommerce. Minimum 15 minutes; maximum 60 minutes.',
                    'default'           => '30',
                    'desc_tip'          => true,
                ...
                'enable_website_product_sync' => array(
                    'title'       => 'Website to POS Sync',
                    'type'        => 'checkbox',
                    'label'       => 'Create new WooCommerce products in Tuma POS automatically',
                    'description' => 'When POS sync is enabled, newly created website products and their variations are sent to Tuma POS immediately.',
                    'default'     => 'yes',
                    'desc_tip'    => true,
                ),
                'sync_products_button' => array(
                    ...
                    'description' => 'Choose the sync direction. Importing WooCommerce to POS is useful when the website existed before adopting Tuma POS.',
                ),
                'sms_section' => array(
        ...
        public function thankyou_page($order_id) {
            ...
            if ($order->get_status() === 'pending') {
                ...
                echo '<input type="hidden" id="current_order_key" value="' . esc_attr($order->get_order_key()) . '" />';
            ...
            <tr valign="top">
                ...
                <td class="forminp">
                    <div id="tuma-sync-products-status">
                        <button type="button" id="sync-tuma-products" class="button-secondary">Import POS to WooCommerce</button>
                        <button type="button" id="import-woo-products" class="button-primary" style="margin-left: 8px;">Import WooCommerce to POS</button>
            ...
            jQuery(document).ready(function($) {
                $('#sync-tuma-products').on('click', function() {
                    ...
                    $.ajax({
                        ...
                        complete: function() {
                            button.prop('disabled', false).text('Import POS to WooCommerce');
                        }
                    });
                });

                $('#import-woo-products').on('click', function() {
                    var button = $(this);
                    var result = $('#tuma-sync-result');

                    button.prop('disabled', true).text('Importing...');
                    result.html('<div style="color: #666;">Importing WooCommerce products and stock into Tuma POS...</div>');

                    $.ajax({
                        url: ajaxurl,
                        type: 'POST',
                        data: {
                            action: 'tuma_import_products_to_pos',
                            nonce: '<?php echo wp_create_nonce('tuma_import_products_to_pos'); ?>'
                        },
                        success: function(response) {
                            if (response.success) {
                                var message = '<strong>✓ Import completed</strong><br>Products synced: ' + response.data.synced + '<br>Products created: ' + response.data.created + '<br>Products updated: ' + response.data.updated;
                                if (response.data.errors && response.data.errors.length) {
                                    message += '<br><strong>Warnings:</strong><br>' + response.data.errors.join('<br>');
                                }
                                result.html('<div style="color: green; padding: 10px; background: #f0f8f0; border: 1px solid #4CAF50; border-radius: 4px;">' + message + '</div>');
                            } else {
                                result.html('<div style="color: red; padding: 10px; background: #fff0f0; border: 1px solid #f44336; border-radius: 4px;"><strong>✗ Import failed</strong><br>' + response.data.message + '</div>');
                            }
                        },
                        error: function() {
                            result.html('<div style="color: red;">Import failed. Please try again.</div>');
                        },
                        complete: function() {
                            button.prop('disabled', false).text('Import WooCommerce to POS');
                        }
                    });
                });
            });
        ...
        public function process_admin_options() {
            ...
            // Persist the effective value as well as clamping it at runtime, so
            // invalid/manual option edits cannot schedule outside 15-60 minutes.
            $settings = get_option('woocommerce_tuma_payments_settings', array());
            $requested_interval = isset($settings['product_sync_interval']) ? absint($settings['product_sync_interval']) : 30;
            $settings['product_sync_interval'] = (string) max(15, min(60, $requested_interval ?: 30));
            update_option('woocommerce_tuma_payments_settings', $settings);
            $this->settings = $settings;
            tuma_reschedule_product_sync();
        ...
        public function payment_scripts() {
            ...
            wp_add_inline_script('tuma-payment-js', 'var TUMA_PDF_RECEIPT_URL = "' . esc_url_raw(home_url('wc-api/tuma_pdf_receipt')) . '"', 'before');
        ...
        /**
         * Show a PDF receipt action only after WooCommerce considers the order paid.
         */
        public function show_receipt_download($order) {
            if (is_numeric($order)) {
            ...
            echo '<p class="tuma-pdf-receipt-action"><a class="button" href="' . esc_url($this->receipt_download_url($order)) . '">Download payment receipt (PDF)</a></p>';
        }

        private function receipt_download_url($order) {
            return add_query_arg(array(
                'order' => $order->get_id(),
                'key'   => $order->get_order_key(),
            ), home_url('wc-api/tuma_pdf_receipt'));
        }

        /**
         * Serve a paid order receipt after validating ownership/order key.
         */
        public function download_pdf_receipt() {
            ...
            if (!$order || !$order->is_paid() || $order->get_payment_method() !== $this->id) {
                status_header(404);
                exit('Paid receipt not found.');
            }

            $customer_id = (int) $order->get_customer_id();
            $owns_order = $customer_id > 0 && get_current_user_id() === $customer_id;
            ...
            if (!$valid_key && !$owns_order && !$is_manager) {
                status_header(403);
                exit('You are not allowed to download this receipt.');
            }
            ...
            $pdf = Tuma_PDF_Receipt::generate(array(
                ...
                'customer_email' => $order->get_billing_email(),
                'date'           => wc_format_datetime($paid_date, 'd M Y, H:i'),
                'currency'       => $order->get_currency(),
                'subtotal'       => max(0, (float) $order->get_total() - (float) $order->get_shipping_total() - (float) $order->get_shipping_tax()),
            ...
            header('Content-Disposition: attachment; filename="tuma-receipt-' . sanitize_file_name($order->get_order_number()) . '.pdf"');
...
add_filter('woocommerce_my_account_my_orders_actions', function($actions, $order) {
    if ($order && $order->is_paid() && $order->get_payment_method() === 'tuma_payments') {
        $actions['tuma_pdf_receipt'] = array(
            'url'  => add_query_arg(array('order' => $order->get_id(), 'key' => $order->get_order_key()), home_url('wc-api/tuma_pdf_receipt')),
...
// One-click initial migration from an existing WooCommerce catalog to Tuma POS.
add_action('wp_ajax_tuma_import_products_to_pos', 'handle_tuma_import_products_to_pos');

function handle_tuma_import_products_to_pos() {
    check_ajax_referer('tuma_import_products_to_pos', 'nonce');

    if (!current_user_can('manage_woocommerce')) {
        wp_send_json_error(array('message' => 'Permission denied'), 403);
    }

    $result = tuma_sync_products_to_pos();
    if ($result['success']) {
        wp_send_json_success($result);
    }
    wp_send_json_error($result);
...
/**
 * Push a complete WooCommerce catalog into Tuma POS.
 */
function tuma_sync_products_to_pos() {
    $token = tuma_get_api_token();
    if (!$token) {
        return array('success' => false, 'message' => 'Failed to authenticate with Tuma API. Check your credentials.');
    }
    ...
    do {
        ...
        foreach ($products as $product) {
            $result = tuma_sync_woocommerce_product_to_pos($product, $token, true);
            if ($result['success']) {
                $synced++;
                $result['created'] ? $created++ : $updated++;
            } else {
                $errors[] = esc_html($product->get_name() . ': ' . $result['message']);
            }
        }
...
/**
 * Automatically create newly added website products in POS. The update hook is
 * a fallback for stores where WooCommerce fires the new-product hook before all
 * product fields have been persisted.
 */
add_action('woocommerce_new_product', 'tuma_sync_new_woocommerce_product_to_pos', 30, 2);
add_action('woocommerce_update_product', 'tuma_sync_new_woocommerce_product_to_pos', 30, 2);

function tuma_sync_new_woocommerce_product_to_pos($product_id, $product = null) {
    ...
    $settings = get_option('woocommerce_tuma_payments_settings', array());
    $enabled = isset($settings['enable_pos_sync']) && $settings['enable_pos_sync'] === 'yes';
    $website_sync = !isset($settings['enable_website_product_sync']) || $settings['enable_website_product_sync'] === 'yes';
    if (!$enabled || !$website_sync) {
        return;
    }
    ...
    $needs_sync = !$product->get_meta('_tuma_product_id');
    if (!$needs_sync && $product->is_type('variable')) {
        foreach ($product->get_children() as $variation_id) {
            $variation = wc_get_product($variation_id);
            if ($variation && !$variation->get_meta('_tuma_variant_id')) {
                $needs_sync = true;
                break;
            }
        }
    ...
    $token = tuma_get_api_token();
    if (!$token) {
        error_log('Tuma website product sync: authentication failed for WooCommerce product ' . $product_id);
        return;
    }

    $syncing[$product_id] = true;
    $result = tuma_sync_woocommerce_product_to_pos($product, $token, false);
    unset($syncing[$product_id]);

    if (!$result['success']) {
        error_log('Tuma website product sync failed for product ' . $product_id . ': ' . $result['message']);
    }
}

/**
 * Create or update one WooCommerce product and its variations in Tuma POS.
 */
function tuma_sync_woocommerce_product_to_pos($product, $token, $force_update = false) {
    if (!$product || (!$product->is_type('simple') && !$product->is_type('variable'))) {
        return array('success' => false, 'created' => false, 'message' => 'Only simple and variable products are supported.');
    }

    $is_variable = $product->is_type('variable');
    $stock = $product->managing_stock() ? (int) $product->get_stock_quantity() : 0;
    if ($is_variable && !$product->managing_stock()) {
        $stock = 0;
        foreach ($product->get_children() as $variation_id) {
            $variation = wc_get_product($variation_id);
    ...
    $payload = array(
        ...
        'is_active'    => $product->get_status() === 'publish',
        'api_channel'  => true,
    );
    ...
    if ($is_variable) {
        ...
        foreach ($product->get_children() as $variation_id) {
            ...
            $variant_sku = trim((string) $variation->get_sku());
            if ($variant_sku === '') {
                $variant_sku = tuma_generated_sku($variation->get_id());
            }
            $woo_variations[$variant_sku] = $variation;
            $payload['variants'][] = array(
                'id'                => $variation->get_meta('_tuma_variant_id') ?: null,
                'sku'               => $variant_sku,
                'price'             => (float) ($variation->get_price() !== '' ? $variation->get_price() : $payload['price']),
                'stock'             => max(0, (int) ($variation->managing_stock() ? $variation->get_stock_quantity() : 0)),
                'is_active'         => $variation->get_status() === 'publish',
                'variation_details' => wp_strip_all_tags(wc_get_formatted_variation($variation, true, false, true)),
            );
        }
    }

    $tuma_id = $product->get_meta('_tuma_product_id');
    if (!$tuma_id && $payload['sku'] !== '') {
        $matched = tuma_find_pos_product_by_sku($token, $payload['sku']);
        $tuma_id = $matched ? $matched['id'] : false;
    }

    $created = !$tuma_id;
    $endpoint = 'https://api.tuma.co.ke/products' . ($tuma_id ? '/' . rawurlencode($tuma_id) : '');
    $response = tuma_pos_api_request($endpoint, $token, $payload, $tuma_id ? 'PUT' : 'POST');

    // A parallel save can win the SKU race; resolve it to the existing POS item.
    if (!$response['success'] && !$tuma_id && $payload['sku'] !== '') {
        $matched = tuma_find_pos_product_by_sku($token, $payload['sku']);
        if ($matched) {
            $tuma_id = $matched['id'];
            $created = false;
            $response = tuma_pos_api_request('https://api.tuma.co.ke/products/' . rawurlencode($tuma_id), $token, $payload, 'PUT');
        }
    }

    if (!$response['success']) {
        return array('success' => false, 'created' => $created, 'message' => $response['message']);
    }

    if (!$tuma_id) {
        $tuma_id = $response['body']['data']['product']['id'] ?? null;
    }
    if (!$tuma_id) {
        return array('success' => false, 'created' => $created, 'message' => 'Tuma API did not return a product ID.');
    }

    $product->update_meta_data('_tuma_product_id', $tuma_id);
    ...
    if ($is_variable) {
        foreach (($response['body']['data']['product']['variants'] ?? array()) as $pos_variant) {
            $variant_sku = isset($pos_variant['sku']) ? (string) $pos_variant['sku'] : '';
            if ($variant_sku !== '' && isset($woo_variations[$variant_sku]) && !empty($pos_variant['id'])) {
                $woo_variations[$variant_sku]->update_meta_data('_tuma_variant_id', $pos_variant['id']);
                $woo_variations[$variant_sku]->update_meta_data('_tuma_last_sync', current_time('mysql'));
                $woo_variations[$variant_sku]->save_meta_data();
            }
        }
    }

    return array('success' => true, 'created' => $created, 'tuma_id' => $tuma_id);
}

function tuma_generated_sku($product_id) {
    return 'WC-' . strtoupper(substr(md5(home_url('/')), 0, 8)) . '-' . absint($product_id);
}

function tuma_find_pos_product_by_sku($token, $sku) {
    $response = wp_remote_get(add_query_arg(array('search' => $sku, 'limit' => 100), 'https://api.tuma.co.ke/products'), array(
    ...
    $body = json_decode(wp_remote_retrieve_body($response), true);
    foreach (($body['data']['products'] ?? array()) as $item) {
        if (empty($item['is_variant']) && isset($item['sku']) && (string) $item['sku'] === (string) $sku) {
            return $item;
        }
    }
    return false;
...
function tuma_pos_api_request($url, $token, array $payload, $method) {
    ...
    if (is_wp_error($response)) {
        return array('success' => false, 'message' => $response->get_error_message(), 'body' => array());
    }
    $body = json_decode(wp_remote_retrieve_body($response), true);
    $status = wp_remote_retrieve_response_code($response);
    return array(
        'success' => $status >= 200 && $status < 300 && !empty($body['success']),
        'message' => $body['message'] ?? ('Tuma API returned HTTP ' . $status),
        'body'    => is_array($body) ? $body : array(),
    );
}

// Configurable stock/product sync (15-60 minutes, default 30).
add_filter('cron_schedules', 'tuma_add_product_sync_cron_schedule');
function tuma_add_product_sync_cron_schedule($schedules) {
    $minutes = tuma_product_sync_interval_minutes();
    $schedules['tuma_product_sync_custom'] = array(
        'interval' => $minutes * MINUTE_IN_SECONDS,
        'display'  => sprintf('Every %d minutes (Tuma product sync)', $minutes),
    );
    return $schedules;
}

function tuma_product_sync_interval_minutes() {
    $settings = get_option('woocommerce_tuma_payments_settings', array());
    $minutes = isset($settings['product_sync_interval']) ? absint($settings['product_sync_interval']) : 30;
    return max(15, min(60, $minutes ?: 30));
}
...
function tuma_reschedule_product_sync() {
    ...
    if (isset($settings['enable_product_sync']) && $settings['enable_product_sync'] === 'yes') {
        wp_schedule_event(time() + MINUTE_IN_SECONDS, 'tuma_product_sync_custom', 'tuma_hourly_product_sync_event');
    }
}

// Migrates existing hourly schedules and restores a missing schedule after updates.
add_action('init', 'tuma_ensure_product_sync_schedule');
function tuma_ensure_product_sync_schedule() {
    $settings = get_option('woocommerce_tuma_payments_settings', array());
    $enabled = isset($settings['enable_product_sync']) && $settings['enable_product_sync'] === 'yes';
    $scheduled = wp_next_scheduled('tuma_hourly_product_sync_event');
    $schedule = $scheduled ? wp_get_schedule('tuma_hourly_product_sync_event') : false;

    if ($enabled && (!$scheduled || $schedule !== 'tuma_product_sync_custom')) {
        tuma_reschedule_product_sync();
...
function tuma_sync_products_from_pos() {
    ...
    while ($has_next) {
        ...
        if ($status_code < 200 || $status_code >= 300) {
            return array(
                'success' => false,
                'message' => isset($results['message']) ? $results['message'] : 'Tuma API returned HTTP ' . $status_code,
                'synced' => $synced,
                'created' => $created,
                'updated' => $updated,
                'errors' => $errors,
